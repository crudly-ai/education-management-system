...
import { StudentEditModalProps } from './types';

export function StudentEditModal({ open, onOpenChange, student, ...props }: StudentEditModalProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (student) {
            put(`/students/${student.id}`, {
                ...
                onError: () => {
                    toast.error(t('Failed to update student'), { description: t('Please check the form and try again') });
                },
            });
        }
    };
        ...
        <Modal open={open} onOpenChange={handleClose}>
            <ModalContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
                <form onSubmit={handleSubmit}>
                    <ModalHeader>
                        <ModalTitle>{t('Edit Student')}</ModalTitle>
                    </ModalHeader>
                    ...
                    <div className="space-y-6 py-4">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="class_id">{t('Class Id')}</Label>
                            <Select value={data.class_id?.toString()} onValueChange={(value) => setData('class_id', parseInt(value))}>
                                <SelectTrigger>
                                    <SelectValue placeholder={t('Select Class')} />
                                </SelectTrigger>
                                <SelectContent>
                                    {relationshipData.classes?.map((classItem) => (
