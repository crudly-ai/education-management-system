...
import { SubjectEditModalProps } from './types';

export function SubjectEditModal({ open, onOpenChange, subject, ...props }: SubjectEditModalProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (subject) {
            put(`/subjects/${subject.id}`, {
                ...
                onError: () => {
                    toast.error(t('Failed to update subject'), { description: t('Please check the form and try again') });
                },
            });
        }
    };
        ...
        <Modal open={open} onOpenChange={handleClose}>
            <ModalContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
                <form onSubmit={handleSubmit}>
                    <ModalHeader>
                        <ModalTitle>{t('Edit Subject')}</ModalTitle>
                    </ModalHeader>
