...
import { ResultCreateModalProps } from './types';

export function ResultCreateModal({ open, onOpenChange, ...props }: ResultCreateModalProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post('/results', {
            onSuccess: () => {
                toast.success(t('Result created successfully'));
                reset();
                onOpenChange(false);
                window.dispatchEvent(new CustomEvent('refreshDataTable'));
            },
            onError: () => {
                toast.error(t('Failed to create result'), { description: t('Please check the form and try again') });
            },
        });
    };
        ...
        <Modal open={open} onOpenChange={handleClose}>
            <ModalContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
                <form onSubmit={handleSubmit}>
                    <ModalHeader>
                        <ModalTitle>{t('Create Result')}</ModalTitle>
                    </ModalHeader>
